#!/bin/bash
# =================================================================
#   RESET COMPLETO DA VM — Gerência de Configuração de Software  
#   Isso vai apagar TUDO relacionado ao projeto                  
# =================================================================
set -e
...
sudo docker images --format "{{.Repository}} {{.ID}}" | grep -E "financeiro|flyway|gerencia" | awk '{print $2}' | xargs -r sudo docker rmi -f 2>/dev/null || true
...
echo -e "${BLUE}curl -fsSL https://raw.githubusercontent.com/Alissa-T/task2_larissa/main/scripts/setup-vm.sh | bash${CLEAR}\n"
