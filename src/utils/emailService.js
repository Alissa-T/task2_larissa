...
const sendEmail = async (to, subject, text, html) => {
	try {
		if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
			console.warn(
				"⚠️ [Email Service] Credenciais do Gmail ausentes no .env (EMAIL_USER e EMAIL_PASS). O email não será enviado.",
			);
			return false;
		}
		...
		if (!to) {
			console.warn(
				"⚠️ [Email Service] Destinatário vazio. O email não será enviado.",
			);
			return false;
		}
		...
		console.log(
			`✉️ [Email Service] Email enviado para ${to}: ${info.messageId}`,
		);
		return true;
	} catch (error) {
