...
const exportarIndividual = async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=lancamento_${l.id}.pdf`);
    doc.pipe(res);
    ...
    doc.fontSize(11).fill('#ddd6fe').font('Helvetica')
      .text('Sistema Financeiro', 0, 62, { align: 'center' });

    // ===== LINHA DECORATIVA =====
    doc.moveTo(50, 115).lineTo(545, 115).strokeColor('#a78bfa').lineWidth(1).stroke();
    ...
    const fields = [
      ...
      { label: 'Valor', value: `${l.tipo_lancamento === 'despesa' ? '- ' : '+ '}${formatCurrencyBR(l.valor)}` },
      { label: 'Tipo', value: l.tipo_lancamento.charAt(0).toUpperCase() + l.tipo_lancamento.slice(1) },
    ...
    fields.forEach((field) => {
      // Label
      doc.fontSize(10).font('Helvetica-Bold').fill('#6b7280')
      ...
      if (field.label === 'Valor') {
        ...
        doc.fontSize(12).font('Helvetica-Bold').fill(color)
          .text(field.value, valueX, y);
      } else if (field.label === 'Tipo') {
        const color = l.tipo_lancamento === 'receita' ? '#059669' : '#dc2626';
        doc.fontSize(11).font('Helvetica').fill(color)
          .text(field.value, valueX, y);
      } else {
        doc.fontSize(11).font('Helvetica').fill('#1e1b4b')
          .text(field.value, valueX, y);
      }
    ...
    // ===== LINHA INFERIOR =====
    y += 15;
    doc.moveTo(50, y).lineTo(545, y).strokeColor('#a78bfa').lineWidth(1).stroke();

    // ===== RODAPÉ =====
    y += 15;
    doc.fontSize(8).font('Helvetica-Oblique').fill('#9ca3af')
      .text(`Documento gerado em ${new Date().toLocaleString('pt-BR')}`, 0, y, { align: 'center' });
...
const exportarTodos = async (req, res) => {
  try {
    ...
    doc.fontSize(10).fill('#ddd6fe').font('Helvetica')
    ...
    // Header da tabela
    doc.rect(startX, tableY, colWidths.reduce((a, b) => a + b, 0) + 80, 28).fill('#7c3aed');

    let hx = startX + 10;
    headers.forEach((header, i) => {
      doc.fontSize(9).font('Helvetica-Bold').fill('#ffffff')
        .text(header, hx, tableY + 9, { width: colWidths[i], align: i === 3 ? 'right' : 'left' });
      hx += colWidths[i] + 16;
    });
    ...
    lancamentos.forEach((l, index) => {
      ...
      if (tableY > 480) {
        ...
        headers.forEach((header, i) => {
          doc.fontSize(9).font('Helvetica-Bold').fill('#ffffff')
            .text(header, hx2, tableY + 9, { width: colWidths[i], align: i === 3 ? 'right' : 'left' });
          hx2 += colWidths[i] + 16;
        });
        tableY += 28;
      }
      ...
      // Fundo alternado
      if (index % 2 === 0) {
        doc.rect(startX, tableY, colWidths.reduce((a, b) => a + b, 0) + 80, 26).fill('#f5f3ff');
      } else {
        doc.rect(startX, tableY, colWidths.reduce((a, b) => a + b, 0) + 80, 26).fill('#ffffff');
      }

      // Linha de separação
      doc.moveTo(startX, tableY + 26)
      ...
      // ID
      doc.fontSize(9).font('Helvetica').fill('#1e1b4b')
      ...
      // Data
      doc.fontSize(9).font('Helvetica').fill('#1e1b4b')
        .text(formatDateBR(l.data_lancamento), rx, tableY + 8, { width: colWidths[2] });
      rx += colWidths[2] + 16;

      // Valor (colorido)
      const valorColor = l.tipo_lancamento === 'receita' ? '#059669' : '#dc2626';
      const valorPrefix = l.tipo_lancamento === 'despesa' ? '- ' : '+ ';
      doc.fontSize(9).font('Helvetica-Bold').fill(valorColor)
        .text(`${valorPrefix}${formatCurrencyBR(l.valor)}`, rx, tableY + 8, { width: colWidths[3], align: 'right' });
      rx += colWidths[3] + 16;

      // Tipo (colorido)
      const tipoColor = l.tipo_lancamento === 'receita' ? '#059669' : '#dc2626';
      doc.fontSize(9).font('Helvetica').fill(tipoColor)
        .text(l.tipo_lancamento.charAt(0).toUpperCase() + l.tipo_lancamento.slice(1), rx, tableY + 8, { width: colWidths[4] });
    ...
    doc.roundedRect(startX, tableY, boxWidth, 50, 5)
      .fill('#f5f3ff')
    ...
    doc.fontSize(12).font('Helvetica-Bold').fill(saldoColor)
