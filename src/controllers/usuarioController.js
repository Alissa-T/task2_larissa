...
const criar = async (req, res) => {
	try {
		...
		if (!nome || !login || !senha || !email) {
			return res
				.status(400)
				.json({ error: "Nome, login, senha e email são obrigatórios" });
		}
		...
		const existingEmail = await pool.query(
			"SELECT id FROM usuario WHERE email = $1 AND situacao = 'ativo'",
			[email],
		);
...
const atualizar = async (req, res) => {
	try {
		...
		if (!nome || !login || !email) {
			return res
				.status(400)
				.json({ error: "Nome, login e email são obrigatórios" });
		}
...
const excluir = async (req, res) => {
	try {
		...
		if (req.session?.usuario && req.session.usuario.id === parseInt(id, 10)) {
			return res
				.status(400)
